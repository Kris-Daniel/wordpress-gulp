		...
		public function __construct( Wbcr_Factory414_Plugin $plugin ) {
			...
			if ( $this->add_link_to_plugin_actions ) {
				if ( $plugin->isNetworkActive() ) {
					...
					add_filter( "network_admin_plugin_action_links_" . $this->plugin->get_paths()->basename, array(
						$this,
						'addLinkToPluginActions'
					) );
				} else {
					// plugin settings link
					add_filter( "plugin_action_links_" . $this->plugin->get_paths()->basename, array(
						$this,
						'addLinkToPluginActions'
					) );
				}
